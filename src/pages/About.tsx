...
const About = () => {
  ...
  return (
    <section ref={sections.about} id='about' className='py-16 lg:py-0 w-full px-4 sm:px-8 sm:max-w-3xl md:max-w-6xl xl:max-w-7xl mx-auto flex flex-col-reverse items-center justify-center gap-3 lg:gap-5 xl:gap-10 lg:flex-row lg:mb-10 xl:mb-0 lg:min-h-[500px]'>
      {/* intro */}
        ...
        <h2 ref={titleRef} className="w-full mb-3 text-lg sm:text-xl md:text-2xl lg:text-3xl xl:text-5xl leading-8 md:leading-12 font-bold relative z-12">
          {titleTexts.map((text, index) => (
            <p key={index} className={clsx(
              ...
              index < 2 ? 'text-2xl text-t-primary' : 'lg:text-3xl xl:text-5xl text-t-sub',
              index % 2 === 0 ? "mb-1" : "mb-5"
            )}>{text}</p>
          ))}
        </h2>
