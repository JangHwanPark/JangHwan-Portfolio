...
const Tab = ({ tabKey, className, children }: TabProps) => {
  ...
  const classes = clsx(
    className,
    'relative xs:text-base lg:text-xl transition-all',
    isActive
      ? 'border-t-2 border-x-2 border-c-yellow font-bold text-white px-8 py-3 lg:px-10 rounded-t-xl'
      : 'px-0 text-primary bg-c-yellow rounded-2xl mx-3 px-5 py-2 bg-gradient-to-b from-[#F4B400] to-white font-semibold'
  );
  // 바텀 부분 둥글게 작업중
